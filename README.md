...
| `retrieve_docs(self, query: str, metadata_filter: dict | None = None)` | Retrieves documents matching a query, optionally filtered by metadata. | - `query: str`: Query string<br>- `metadata_filter: dict | None = None`: Optional metadata filter | List of tuples `(Document, float)` |
